...
package zephyr_sys_device_mmio_h is

   --  arg-macro: procedure Z_DEVICE_MMIO_ROM_INITIALIZER (node_id)
   --    { .phys_addr := DT_REG_ADDR(node_id), .size := DT_REG_SIZE(node_id) }
   --  arg-macro: procedure Z_DEVICE_MMIO_NAMED_ROM_INITIALIZER (name, node_id)
   --    { .phys_addr := DT_REG_ADDR_BY_NAME(node_id, name), .size := DT_REG_SIZE_BY_NAME(node_id, name) }
   --  unsupported macro: DEVICE_MMIO_RAM mm_reg_t _mmio
   --  arg-macro: function DEVICE_MMIO_RAM_PTR (device)
   --    return mm_reg_t *)((device).data;
   --  unsupported macro: DEVICE_MMIO_ROM struct z_device_mmio_rom _mmio
   --  arg-macro: function DEVICE_MMIO_ROM_PTR (dev)
   --    return (struct z_device_mmio_rom *)((dev).config);
   --  arg-macro: procedure DEVICE_MMIO_ROM_INIT (node_id)
   --    ._mmio := Z_DEVICE_MMIO_ROM_INITIALIZER(node_id)
   --  arg-macro: procedure DEVICE_MMIO_MAP (dev, flags)
   --    device_map(DEVICE_MMIO_RAM_PTR(dev), DEVICE_MMIO_ROM_PTR(dev).phys_addr, DEVICE_MMIO_ROM_PTR(dev).size, (flags))
   --  arg-macro: function DEVICE_MMIO_GET (dev)
   --    return *DEVICE_MMIO_RAM_PTR(dev);
   --  arg-macro: procedure DEVICE_MMIO_NAMED_RAM (name)
   --    mm_reg_t name
   --  arg-macro: function DEVICE_MMIO_NAMED_RAM_PTR (dev, name)
   --    return and(DEV_DATA(dev).name);
   --  arg-macro: procedure DEVICE_MMIO_NAMED_ROM (name)
   --    struct z_device_mmio_rom name
   --  arg-macro: function DEVICE_MMIO_NAMED_ROM_PTR (dev, name)
   --    return and(DEV_CFG(dev).name);
   --  arg-macro: procedure DEVICE_MMIO_NAMED_ROM_INIT (name, node_id)
   --    .name := Z_DEVICE_MMIO_ROM_INITIALIZER(node_id)
   --  arg-macro: procedure DEVICE_MMIO_NAMED_ROM_INIT_BY_NAME (name, node_id)
   --    .name := Z_DEVICE_MMIO_NAMED_ROM_INITIALIZER(name, node_id)
   --  arg-macro: procedure DEVICE_MMIO_NAMED_MAP (dev, name, flags)
   --    device_map(DEVICE_MMIO_NAMED_RAM_PTR((dev), name), (DEVICE_MMIO_NAMED_ROM_PTR((dev), name).phys_addr), (DEVICE_MMIO_NAMED_ROM_PTR((dev), name).size), (flags))
   --  arg-macro: function DEVICE_MMIO_NAMED_GET (dev, name)
   --    return *DEVICE_MMIO_NAMED_RAM_PTR((dev), name);
   --  arg-macro: procedure Z_TOPLEVEL_ROM_NAME (name)
   --    _CONCAT(z_mmio_rom__, name)
   --  arg-macro: procedure Z_TOPLEVEL_RAM_NAME (name)
   --    _CONCAT(z_mmio_ram__, name)
   --  arg-macro: procedure DEVICE_MMIO_TOPLEVEL (name, node_id)
   --    __pinned_bss mm_reg_t Z_TOPLEVEL_RAM_NAME(name); __pinned_rodata const struct z_device_mmio_rom Z_TOPLEVEL_ROM_NAME(name) := Z_DEVICE_MMIO_ROM_INITIALIZER(node_id)
   --  arg-macro: procedure DEVICE_MMIO_TOPLEVEL_DECLARE (name)
   --    extern mm_reg_t Z_TOPLEVEL_RAM_NAME(name); extern const struct z_device_mmio_rom Z_TOPLEVEL_ROM_NAME(name)
   --  arg-macro: procedure DEVICE_MMIO_TOPLEVEL_STATIC (name, node_id)
   --    __pinned_bss static mm_reg_t Z_TOPLEVEL_RAM_NAME(name); __pinned_rodata static const struct z_device_mmio_rom Z_TOPLEVEL_ROM_NAME(name) := Z_DEVICE_MMIO_ROM_INITIALIZER(node_id)
   --  arg-macro: procedure DEVICE_MMIO_TOPLEVEL_RAM_PTR (name)
   --    andZ_TOPLEVEL_RAM_NAME(name)
   --  arg-macro: procedure DEVICE_MMIO_TOPLEVEL_ROM_PTR (name)
   --    andZ_TOPLEVEL_ROM_NAME(name)
   --  arg-macro: procedure DEVICE_MMIO_TOPLEVEL_MAP (name, flags)
   --    device_map(andZ_TOPLEVEL_RAM_NAME(name), Z_TOPLEVEL_ROM_NAME(name).phys_addr, Z_TOPLEVEL_ROM_NAME(name).size, (flags))
   --  arg-macro: function DEVICE_MMIO_TOPLEVEL_GET (name)
   --    return (mm_reg_t)Z_TOPLEVEL_RAM_NAME(name);
  ...
  --* MMIO physical address  
   type z_device_mmio_rom is record
      phys_addr : aliased sys_ustdint_h.uintptr_t;  -- /Users/jgrivera/my-projects/third-party/zephyrproject/zephyr/include/zephyr/sys/device_mmio.h:58
      size : aliased stddef_h.size_t;  -- /Users/jgrivera/my-projects/third-party/zephyrproject/zephyr/include/zephyr/sys/device_mmio.h:61
   end record
   with Convention => C_Pass_By_Copy;  -- /Users/jgrivera/my-projects/third-party/zephyrproject/zephyr/include/zephyr/sys/device_mmio.h:56
  ...
  --  
   ...
   procedure device_map
     (virt_addr : access zephyr_sys_sys_io_h.mm_reg_t;
      phys_addr : sys_ustdint_h.uintptr_t;
      size : stddef_h.size_t;
      flags : sys_ustdint_h.uint32_t)  -- /Users/jgrivera/my-projects/third-party/zephyrproject/zephyr/include/zephyr/sys/device_mmio.h:97
   with Import => True, 
        Convention => C, 
        External_Name => "device_map";
