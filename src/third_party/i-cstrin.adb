...
is
   ...
   begin
      ...
      --  ZEPHYR_CHANGE: TO_C function not supported because Secondary stack
      --  are  not supported
      --  Update (Item, Offset, To_C (Str, Append_Nul => False), Check);
      To_C (Str, C_Str, Count, Append_Nul => True);
      Update (Item, Offset, C_Str, Check);
   end Update;
