...
--  This is the Zephyr RTOS version of this package for the Jorvik profile
...
package body System.Tasking.Restricted.Stages is
   ...
   --  body, and which in turn calls the compiler-generated task body
   ...
   begin
      ...
      --  the creator. This is called before the task body starts executing.

      Enter_Task (Self_ID);

      --  Call the compiler-generated task body procedure
      ...
      --  Mark the task as terminated and suspend it indefinitely.
      --  In Ravenscar, tasks should never terminate, so this is an error
      --  condition, but we handle it gracefully by putting the task to sleep.
   ...
   begin
      ...
      --  Creation of the Zephyr thread was deferred until activation.
      --  So create it now.
      ...
      while C /= Null_Task loop
         ...
         --  Create the underlying Zephyr thread for this Ada task
         ...
         if Success then
            ...
            raise Program_Error with "Failed to create Zephyr thread for task";
         end if;
   ...
   begin
      --  In Ravenscar/Jorvik, tasks are activated immediately when created
   ...
   begin
      ...
      --  Initialize the Ada Task Control Block
      ...
      if not Success then
         raise Program_Error with "Failed to initialize ATCB";
      end if;
   ...
   begin
      ...
      --  Append this task to the global activation chain
   ...
   begin
      raise Program_Error with "Finalize_Global_Tasks should not be called";
...
begin
   --  Initialize the tasking runtime system
   Tasking.Initialize;
