...
--  This is the Ravenscar/Jorvik version for Zephyr
...
package body System.Tasking is
   ...
   --  ATCB for the environment task. The name of this variable is
   ...
   begin
      ...
      --  Initialize T.Common.LL (call Zephyr-specific initialization)
   ...
   begin
      ...
      --  Compute priority for environment task
      ...
      --  Initialize the ATCB for the environment (main) task
      ...
      --  Initialize the task primitives layer
      --  This associates the Zephyr main thread with the environment task

      Task_Primitives.Operations.Initialize
        (Environment_Task'Access);

      --  Note: Priority is already set in Enter_Task via s-taprop.Initialize
      ...
      --  For Zephyr, we currently only support CPU'First as the main CPU

      if Main_CPU /= Unspecified_CPU and then Main_CPU /= Integer (CPU'First)
      then
         --  Invalid CPU, raise Tasking_Error

         raise Tasking_Error with "Main CPU must be CPU'First";
      end if;
   end Initialize;
