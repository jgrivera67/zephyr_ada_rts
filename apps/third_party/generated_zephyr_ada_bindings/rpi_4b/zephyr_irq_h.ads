...
package zephyr_irq_h is

   --  arg-macro: procedure IRQ_CONNECT (irq_p, priority_p, isr_p, isr_param_p, flags_p)
   --    ARCH_IRQ_CONNECT(irq_p, priority_p, isr_p, isr_param_p, flags_p)
   --  arg-macro: procedure IRQ_DIRECT_CONNECT (irq_p, priority_p, isr_p, flags_p)
   --    ARCH_IRQ_DIRECT_CONNECT(irq_p, priority_p, isr_p, flags_p)
   --  arg-macro: procedure ISR_DIRECT_HEADER ()
   --    ARCH_ISR_DIRECT_HEADER()
   --  arg-macro: procedure ISR_DIRECT_FOOTER (check_reschedule)
   --    ARCH_ISR_DIRECT_FOOTER(check_reschedule)
   --  arg-macro: procedure ISR_DIRECT_PM ()
   --    ARCH_ISR_DIRECT_PM()
   --  arg-macro: procedure ISR_DIRECT_DECLARE (name)
   --    ARCH_ISR_DIRECT_DECLARE(name)
   --  arg-macro: procedure irq_lock ()
   --    arch_irq_lock()
   --  arg-macro: procedure irq_unlock (key)
   --    arch_irq_unlock(key)
   --  arg-macro: procedure irq_enable (irq)
   --    arch_irq_enable(irq)
   --  arg-macro: procedure irq_disable (irq)
   --    arch_irq_disable(irq)
   --  arg-macro: procedure irq_is_enabled (irq)
   --    arch_irq_is_enabled(irq)
  ...
  --  
   ...
   function irq_connect_dynamic
     (irq : unsigned;
      ...
      flags : sys_ustdint_h.uint32_t) return int  -- /Users/jgrivera/my-projects/third-party/zephyrproject/zephyr/include/zephyr/irq.h:65
   with Import => True, 
        Convention => C, 
        External_Name => "irq_connect_dynamic";
  ...
  --  
   ...
   function irq_disconnect_dynamic
     (irq : unsigned;
      ...
      routine : access procedure (arg1 : System.Address);
      parameter : System.Address;
      flags : sys_ustdint_h.uint32_t) return int  -- /Users/jgrivera/my-projects/third-party/zephyrproject/zephyr/include/zephyr/irq.h:90
   with Import => True, 
        Convention => C, 
        External_Name => "irq_disconnect_dynamic";
