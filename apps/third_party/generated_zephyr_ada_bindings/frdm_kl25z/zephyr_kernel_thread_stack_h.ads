...
package zephyr_kernel_thread_stack_h is
   ...
   --  arg-macro: function Z_STACK_PTR_TO_FRAME (type, ptr)
   --    return type *)((ptr) - sizeof(type);
   --  unsupported macro: K_KERNEL_STACK_RESERVED ((size_t)0)
   --  arg-macro: function Z_KERNEL_STACK_SIZE_ADJUST (size)
   --    return ROUND_UP(size, ARCH_STACK_PTR_ALIGN) + K_KERNEL_STACK_RESERVED;
   --  unsupported macro: Z_KERNEL_STACK_OBJ_ALIGN ARCH_STACK_PTR_ALIGN
   --  arg-macro: procedure K_KERNEL_STACK_LEN (size)
   --    ROUND_UP(Z_KERNEL_STACK_SIZE_ADJUST(size), Z_KERNEL_STACK_OBJ_ALIGN)
   --  arg-macro: procedure K_KERNEL_STACK_DECLARE (sym, size)
   --    K_KERNEL_HW_SHADOW_STACK_DECLARE(sym, K_THREAD_HW_SHADOW_STACK_SIZE(size)); extern struct z_thread_stack_element sym(K_KERNEL_STACK_LEN(size))
   --  arg-macro: procedure K_KERNEL_STACK_ARRAY_DECLARE (sym, nmemb, size)
   --    K_KERNEL_HW_SHADOW_STACK_ARRAY_DECLARE(sym, nmemb, K_THREAD_HW_SHADOW_STACK_SIZE(size)); extern struct z_thread_stack_element sym(nmemb)(K_KERNEL_STACK_LEN(size))
   --  arg-macro: procedure K_KERNEL_PINNED_STACK_ARRAY_DECLARE (sym, nmemb, size)
   --    K_KERNEL_HW_SHADOW_STACK_ARRAY_DECLARE(sym, nmemb, K_THREAD_HW_SHADOW_STACK_SIZE(size)); extern struct z_thread_stack_element sym(nmemb)(K_KERNEL_STACK_LEN(size))
   --  arg-macro: procedure Z_KERNEL_STACK_DEFINE_IN (sym, size, lsect)
   --    struct z_thread_stack_element lsect __aligned(Z_KERNEL_STACK_OBJ_ALIGN) sym(K_KERNEL_STACK_LEN(size))
   --  arg-macro: procedure Z_KERNEL_STACK_ARRAY_DEFINE_IN (sym, nmemb, size, lsect)
   --    struct z_thread_stack_element lsect __aligned(Z_KERNEL_STACK_OBJ_ALIGN) sym(nmemb)(K_KERNEL_STACK_LEN(size))
   --  arg-macro: procedure K_KERNEL_STACK_DEFINE (sym, size)
   --    Z_KERNEL_STACK_DEFINE_IN(sym, size, __kstackmem); K_THREAD_HW_SHADOW_STACK_DEFINE(sym, K_THREAD_HW_SHADOW_STACK_SIZE(size))
   --  arg-macro: procedure K_KERNEL_PINNED_STACK_DEFINE (sym, size)
   --    Z_KERNEL_STACK_DEFINE_IN(sym, size, __kstackmem); K_THREAD_HW_SHADOW_STACK_DEFINE(sym, K_THREAD_HW_SHADOW_STACK_SIZE(size))
   --  arg-macro: procedure K_KERNEL_STACK_ARRAY_DEFINE (sym, nmemb, size)
   --    Z_KERNEL_STACK_ARRAY_DEFINE_IN(sym, nmemb, size, __kstackmem); K_THREAD_HW_SHADOW_STACK_ARRAY_DEFINE(sym, nmemb, size)
   --  arg-macro: procedure K_KERNEL_PINNED_STACK_ARRAY_DEFINE (sym, nmemb, size)
   --    Z_KERNEL_STACK_ARRAY_DEFINE_IN(sym, nmemb, size, __kstackmem); K_THREAD_HW_SHADOW_STACK_ARRAY_DEFINE(sym, nmemb, size)
   --  arg-macro: procedure K_KERNEL_STACK_MEMBER (sym, size)
   --    Z_KERNEL_STACK_DEFINE_IN(sym, size,)
   --  arg-macro: function K_KERNEL_STACK_SIZEOF (sym)
   --    return sizeof(sym) - K_KERNEL_STACK_RESERVED;
   --  unsupported macro: K_THREAD_STACK_RESERVED K_KERNEL_STACK_RESERVED
   --  unsupported macro: K_THREAD_STACK_SIZEOF K_KERNEL_STACK_SIZEOF
   --  unsupported macro: K_THREAD_STACK_LEN K_KERNEL_STACK_LEN
   --  unsupported macro: K_THREAD_STACK_DEFINE K_KERNEL_STACK_DEFINE
   --  unsupported macro: K_THREAD_STACK_ARRAY_DEFINE K_KERNEL_STACK_ARRAY_DEFINE
   --  unsupported macro: K_THREAD_STACK_BUFFER K_KERNEL_STACK_BUFFER
   --  unsupported macro: K_THREAD_STACK_DECLARE K_KERNEL_STACK_DECLARE
   --  unsupported macro: K_THREAD_STACK_ARRAY_DECLARE K_KERNEL_STACK_ARRAY_DECLARE
   --  unsupported macro: K_THREAD_PINNED_STACK_DEFINE K_KERNEL_PINNED_STACK_DEFINE
   --  unsupported macro: K_THREAD_PINNED_STACK_ARRAY_DEFINE K_KERNEL_PINNED_STACK_ARRAY_DEFINE
  ...
  --
   ...
   --JGR pragma Compile_Time_Warning (True, "packed layout may be incorrect");
   type z_thread_stack_element is record
      data : char;  -- /Users/jgrivera/my-projects/third-party/zephyrproject/zephyr/include/zephyr/kernel/thread_stack.h:48
   end record
   with Convention => C_Pass_By_Copy,
        Pack => True,
        Alignment => 1;  -- /Users/jgrivera/my-projects/third-party/zephyrproject/zephyr/include/zephyr/kernel/thread_stack.h:47
  ...
  --
   ...
   function z_stack_ptr_align (ptr : Interfaces.C.Strings.chars_ptr) return Interfaces.C.Strings.chars_ptr  -- /Users/jgrivera/my-projects/third-party/zephyrproject/zephyr/include/zephyr/kernel/thread_stack.h:69
   with Import => True,
        Convention => C,
        External_Name => "z_stack_ptr_align";
  ...
  --* @}
   function K_KERNEL_STACK_BUFFER (sym : access z_thread_stack_element) return Interfaces.C.Strings.chars_ptr  -- /Users/jgrivera/my-projects/third-party/zephyrproject/zephyr/include/zephyr/kernel/thread_stack.h:449
   with Import => True,
        Convention => C,
        External_Name => "K_KERNEL_STACK_BUFFER";
...
end zephyr_kernel_thread_stack_h;
